...
describe('Using utilities for entities ->', () => {
  ...
  const receivedAction = { type: CONST_RECEIVE, items: [item] };
  ...
  const receivedEntityAction = { type: CONST_RECEIVE_ENTITY, entity: item };
  ...
  const receivedSavedAction = { type: CONST_ENTITY_SAVED, entity: item };
  ...
  describe('When generating generic constants,', () => {
    ...
    it('the result should contain twelve constants using "generic" entity', () => {
      expect(Object.values(constants).length, 'constants numbers').to.be.equal(12);
  ...
  describe('When generating generic actions,', () => {
    ...
    it('the result should contain twelve actions using constants', () => {
      expect(Object.values(actions).length, 'actions numbers').to.be.equal(12);

      // All
      expect(actions.requestAll(), 'request actions').to.be.deep.equal(requestAction);
      const receivedActionInstance = actions.receiveSome([item]);
      ...
      expect(actions.receiveOne(item), 'receive entity action').to.be.deep.equal(receivedEntityAction);
      ...
      expect(actions.saved(item), 'saved action').to.be.deep.equal(receivedSavedAction);
  ...
  describe('When generating a generic reducer', () => {
    describe('and dispatching a requestAll action,', () => {
      const res = generateEntitiesReducer(initialState, requestAction, entities);
      const expectedState = {
        isFetching: true,
        didInvalidate: false,
        items: {},
    ...
    describe('and dispatching a receiveSome action,', () => {
      const actions = generateEntitiesActions(entities);
      const action = actions.receiveSome([item]);
      const res = generateEntitiesReducer(initialState, action, entities);
      const expectedState = {
        isFetching: false,
        didInvalidate: false,
        items: { testId: item },
    ...
    describe('and dispatching an invalidRequest action,', () => {
      const res = generateEntitiesReducer(initialState, invalidAction, entities);
      const expectedState = {
        isFetching: false,
        didInvalidate: true,
        items: {},
    ...
    describe('and dispatching a requestOne action,', () => {
      const res = generateEntitiesReducer(initialState, requestEntityAction, entities);
      const expectedState = {
        isFetching: false,
        didInvalidate: true,
        items: {
    ...
    describe('and dispatching a revieveOne action,', () => {
      const res = generateEntitiesReducer(initialState, receivedEntityAction, entities);
      const expectedState = {
        isFetching: false,
        didInvalidate: true,
    ...
    describe('and dispatching an invalidRequestEntity action,', () => {
      const res = generateEntitiesReducer(initialState, invalidEntityAction, entities);
      const expectedState = {
        isFetching: false,
        didInvalidate: true,
        items: {
    ...
    describe('and dispatching a requestSaveAction action,', () => {
      const res = generateEntitiesReducer(initialState, requestSaveAction, entities);
      const expectedState = {
        isFetching: false,
        didInvalidate: true,
        items: {
    ...
    describe('and dispatching a receivedSavedAction action,', () => {
      const res = generateEntitiesReducer(initialState, receivedSavedAction, entities);
      const expectedState = {
        isFetching: false,
        didInvalidate: true,
        items: { [item.id]: { ...item, isFetching: false } },
    ...
    describe('and dispatching an invalidSaveAction action,', () => {
      const res = generateEntitiesReducer(initialState, invalidSaveAction, entities);
      const expectedState = {
        isFetching: false,
        didInvalidate: true,
        items: {
    ...
    describe('and dispatching a receivedDeletedAction action,', () => {
      const initState = { ...initialState, items: { [item.id]: item } };
      const res = generateEntitiesReducer(initState, receivedDeletedAction, entities);
      const expectedState = {
        isFetching: false,
        didInvalidate: true,
        items: {},
